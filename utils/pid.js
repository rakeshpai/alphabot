"use strict";
// From https://github.com/Philmod/node-pid-controller
// Modified to handle angles in errors

const { restrictAngle } = require("./");
...
class Controller {
  ...
  update(currentValue) {
    ...
    if(this.angle) error = restrictAngle(error);

    this.sumError = this.sumError + error*dt;
    if(this.angle) this.sumError = restrictAngle(this.sumError);
    ...
    if(this.angle) dError = restrictAngle(dError);
